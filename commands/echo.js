  ...
  async execute(interaction) {
    ...
    if (subcommand === 'text') {
      ...
      // Patikriname, ar tai tekstinis failas (.txt arba .json)
        ...
        if (!isTextFile) {
          const embed = createEmbedFromMessage(getMessage('echo', 'errors.fileFormat'), 'error');
        ...
        // Patikriname failo dydį (iš config.json)
        const maxSizeMB = config?.echo?.maxFileSizeMB ?? 1;
        const maxSize = maxSizeMB * 1024 * 1024;
        if (attachment.size > maxSize) {
          const embed = createEmbedFromMessage(getMessage('echo', 'errors.fileTooLarge').replace('{size}', maxSizeMB.toString()), 'error');
          await safeReply(interaction, {
            embeds: [embed],
            ephemeral: true,
          });
      ...
      try {
        ...
        const embed = createEmbedFromMessage(getMessage('echo', 'errors.fileRead'), 'error');
        await safeReply(interaction, {
          embeds: [embed],
          ephemeral: true,
        });
    ...
    if (!message || message.length === 0) {
      const embed = createEmbedFromMessage(getMessage('echo', 'errors.emptyText'), 'error');
      await safeReply(interaction, {
        embeds: [embed],
        ephemeral: true,
      });
